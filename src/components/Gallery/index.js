...
class Gallery extends Component {
  ...
  getFilteredImages = () => {
    ...
    const filteredImg = imagesList.filter(each => activeImgId === each.id)
    return filteredImg
  }
  ...
  render() {
    ...
    const filteredImg = this.getFilteredImages()
    const {imageUrl, imageAltText} = filteredImg[0]
        ...
        <ul className="thumbnails-container">
          {imagesList.map(each => (
            <ThumbnailItem
              ...
              onThumbnailClicked={this.onThumbnailClick}
              isActive={activeImgId === each.id}
            />
          ))}
        </ul>
      </div>
